$(document).ready(function() {
	...
	//Reseting the playerMaxAbsVelocity
	...
	function animate() {
		...
		//Create Platform Gradiant
		var gradient = context.createRadialGradient(platformX, platformY, platformInnerRadius, platformX, platformY, platformOuterRadius);
		gradient.addColorStop(0,"#ff3300");
		gradient.addColorStop(1, "orange");
		context.fillStyle = gradient;
		context.fill();
		
		//Draw player velocity line
		if (playerSelected) {
			context.strokeStyle = "rgb(247, 188, 97)";
			context.lineWidth = 6;
			context.beginPath();
		...
		//Draw Asteroids		
		context.fillStyle = "rgb(192, 201, 204)";
		...
		for (var i = 0; i < asteroidsLength; i++) {
			...
			for (var j = i+1; j < asteroidsLength; j++) {
				...
				if (distance < tmpAsteroid.radius + tmpAsteroidB.radius) {
					...
					tmpAsteroid.xB = tmpAsteroid.x + (xB * cosine - yB * sine);
					tmpAsteroid.yB = tmpAsteroid.y + (yB * cosine + xB * sine);
		...
		if (deadAsteroidsLength > 0) {
			...
			var remaining = asteroids.length-1; //removes player asteroid from total count
		...
		if (playGame) {
			//Run animation loop again in 33 milliseconds
